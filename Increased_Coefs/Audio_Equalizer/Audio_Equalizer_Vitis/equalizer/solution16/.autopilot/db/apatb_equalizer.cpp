...
{
transaction<16> tr(99);
aesl_fh.read(AUTOTB_TVOUT_PC_gmem, tr.p, tr.tbytes);
if (little_endian()) { tr.reorder(); }
tr.send<2>((char*)__xlx_apatb_param_coefs, 99, 0);
}
#else
try {
static PostCheck<16> pc(AUTOTB_TVOUT_PC_gmem);
pc.psize = 2;
...
transaction<16> tr(99);
__xlx_offset_byte_param_coefs = 0*2;
if (__xlx_apatb_param_coefs) {
  tr.import<2>((char*)__xlx_apatb_param_coefs, 99, 0);
}
...
unsigned char *pos = (unsigned char*)__xlx_apatb_param_coefs + i * 2;
