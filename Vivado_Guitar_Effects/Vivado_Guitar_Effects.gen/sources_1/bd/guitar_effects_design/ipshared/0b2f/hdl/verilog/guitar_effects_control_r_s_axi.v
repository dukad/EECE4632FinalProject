...
#(parameter
    ...
    output wire [9:0]                    distortion_clip_factor,
...
//        bit 9~0 - distortion_clip_factor[9:0] (Read/Write)
//        others  - reserved
// 0x34 : reserved
...
//        bit 9~0 - delay_mult[9:0] (Read/Write)
//        others  - reserved
// 0x54 : reserved
...
//------------------------Local signal-------------------
    ...
    reg  [9:0]                    int_distortion_clip_factor = 'b0;
        ...
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_DISTORTION_CLIP_FACTOR_DATA_0: begin
                    rdata <= int_distortion_clip_factor[9:0];
                end
...
// int_distortion_clip_factor[9:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_distortion_clip_factor[9:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_DISTORTION_CLIP_FACTOR_DATA_0)
            int_distortion_clip_factor[9:0] <= (WDATA[31:0] & wmask) | (int_distortion_clip_factor[9:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_delay_mult[9:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_DELAY_MULT_DATA_0)
            int_delay_mult[9:0] <= (WDATA[31:0] & wmask) | (int_delay_mult[9:0] & ~wmask);
    end
end
