...
typedef struct {
    ...
    XGuitar_effects_uio_map maps[ MAX_UIO_MAPS ];
} XGuitar_effects_uio_info;

/***************** Variable Definitions **************************************/
static XGuitar_effects_uio_info uio_info;
...
static int uio_info_read_name(XGuitar_effects_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XGuitar_effects_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XGuitar_effects_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XGuitar_effects_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XGuitar_effects_Initialize(XGuitar_effects *InstancePtr, const char* InstanceName) {
	XGuitar_effects_uio_info *InfoPtr = &uio_info;
